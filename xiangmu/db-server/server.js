...
app.get(api.FOCUSLIST_URL, (req, res) => {
    let data = require('./data/homeData.json').focusList;
    res.json({
        status: 0,
        message: 'ok',
        data
...
app.get(api.TOPDATA_URL, (req, res) => {
    let { bigPromotionModule, kingKongModule, policyDescList } = require('./data/homeData.json');
    res.json({
        status: 0,
        message: 'ok',
        data: {
...
app.listen(9000);
console.log('run server port:9000');